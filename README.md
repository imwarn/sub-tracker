...
- **CORS**：动态 origin，不用 `*`，通过 `ALLOWED_ORIGIN` 精确控制
- **OTP**：`crypto.getRandomValues` 生成，5 次错误锁定，60s 冷却，5min TTL
- **Session**：UUID token，30 天 TTL，支持主动登出
- **URL 安全**：`safeHref()` 阻止 `javascript:` / `data:` / `vbscript:` 协议
- **XSS 防护**：所有用户输入通过 `esc()` 做 HTML 转义（含日期、数字等防御性转义）
- **导入去重**：基于 id 的 Set 去重，保留原始 id，单次上限 500 条
- **KV 并发**：read-modify-write 无锁，个人单用户场景可接受；多用户并发需迁移到 D1

---

## ❓ 常见问题 (FAQ)

### Q1: 在「偏好设置」中删除了某个预设分类或区域，已添加的历史订阅数据会丢失或异常吗？
**不会。**
* 预设分类与区域仅作为表单录入时的**「下拉建议推荐池」**，与具体订阅数据是完全解耦的。
* 即使在偏好设置中删除了 `域名 / SSL`，已有订阅卡片依然正常展示该分类，图表统计依然正常聚合，全局搜索依然能正常搜出。
* 录入表单下拉框具备智能兜底：会自动将当前所有卡片正在使用的自定义分类加入提示选项。

### Q2: 历史数据中存储的旧版英文分类（如 `Domain`, `AI`, `Streaming`）是如何兼容与迁移的？
**系统内置了代码级平滑映射引擎（`CATEGORY_ALIASES`）：**
* 加载与展示时，系统会自动将 `Domain` 映射为 `域名 / SSL`、`AI` 映射为 `AI 服务`、`Streaming` 映射为 `流媒体` 等；
* 编辑并重新保存该订阅时，数据会自动完成升级为最新的规范中文名；
* 全局搜索支持**中英文双向别名匹配**：输入 `Domain`、`domain`、`域名` 或 `SSL` 均可直接检索出对应服务；
* 无论你在偏好设置中如何增删预设标签，底层静态映射规则始终永久生效。

### Q3: 点击偏好设置中的「恢复默认预设分类 / 区域」会发生什么？
* 偏好设置中的预设标签列表会重置回官方出厂的 11 个分类或 18 个区域；
* 如果你之前自行添加过额外的个性预设（如 `健身 / 运动`），它们会从“预设标签列表”中移除；
* **所有已录入到订阅条目中的数据完全不受影响**；
* 只有点击弹窗底部的【保存设置】后重置才会真正生效，误点时直接点击【取消】或按 `ESC` 即可放弃修改。

### Q4: 货币与汇率管理是如何运作的？
* **ISO 4217 标准**：所有币种严格按照国际 ISO 4217 3 位大写字母代码管理（涵盖 `CNY`, `USD`, `EUR`, `TRY`, `ARS`, `NGN`, `EGP`, `BRL` 等 60+ 种主流及跨区订阅币种）；
* **基准货币（Base Currency）**：支持在偏好设置中指定主统计货币（如 CNY 或 USD），面板总支出与年度预算均基于此动态折算；
* **实时汇率同步**：支持一键联网同步公网最新汇率中间价，也支持针对信用卡手续费或礼品卡汇率手动微调；
* **表单智能检索**：录入时支持按代码（`USD`）、中文名（`美元`、`土耳其里拉`、`日元`）或点选快速匹配带回。

### Q5: 移动端体验如何？是否支持 PWA 添加到主屏幕？
* **原生级体验**：添加/编辑表单与偏好设置弹窗采用**顶部标题与底部操作栏固定锁定、中间表单独立平滑滚动**的三段式视口布局，在移动端呈现贴合底部的 Bottom-Sheet 卡片风格，键盘弹出时不遮挡保存按钮；
* **PWA 原生支持**：内置 Manifest 与 Service Worker，使用手机 Safari 或 Chrome 打开时，点击「添加到主屏幕」即可像独立 App 一样沉浸式使用。
